...
지시사항:
1. web_search로 아래 5가지를 지난 24시간 기준으로 검색해:
   - "코스피 코스닥 증시 뉴스"
   - "나스닥 미국증시 뉴스"
   - "반도체 AI 주식 뉴스"
   - "오늘 핫한 테마주 섹터 뉴스"
...
2. 아래 순서로 메시지를 작성해:
   ...
   [STEP A] 리스크오프 경보 판단
   아래 데이터를 기준으로 시장 긴장도를 판단해:
   - VIX > 25: 주의 / VIX > 30: 경보
   - 금 전일 대비 +1% 이상 급등
   - 엔화(달러/엔) 전일 대비 -1% 이상 급락 (엔강세 = 리스크오프)
   - 유가 전일 대비 +3% 이상 급등 (지정학)
   3개 이상 동시 해당 시 ⚠️ 리스크오프 경보 섹션 출력.
   해당 없으면 이 섹션 생략.

   [STEP B] 지정학/매크로 긴급 이벤트 확인
   검색 결과에서 아래 유형 이벤트가 있으면 영향 분석 포함:
   - 전쟁/군사 충돌/미사일
   - 관세/무역 제재/수출 금지
   - 연준/ECB 긴급 발표
   - 주요국 정상 발언 (트럼프, 시진핑 등)
...
[리스크오프 경보가 있을 때만 출력]
⚠️ 시장 경보: [VIX/금/엔/유가 이상 신호 요약 1줄]
→ 수혜: [방산/금/달러 등] | 주의: [성장주/신흥국 등]
...
[지정학/매크로 긴급 이벤트가 있을 때만 출력]
━━━━ 🌏 지정학 & 매크로 이벤트 ━━━━
📌 이벤트: [사건 요약]
→ 직접 영향: [유가/환율/국채 등 자산 반응]
→ 국내 영향: [코스피/원화/관련 섹터]
→ 수혜 섹터: [방산/에너지/금 등]
→ 피해 섹터: [반도체수출/항공/소비재 등]
